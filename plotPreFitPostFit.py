...
def plotPreFitPostFit(region):
  ...
  f_data = TFile(os.path.join(os.environ['DMS'],"mono-x.root"),"READ")
  ...
  c.SaveAs(r"/local/ballen/Dropbox (MIT)/Plots/Limits/"+str(sys.argv[1])+"_prefit_postfit_"+region+".pdf")
  c.SaveAs(r"/local/ballen/Dropbox (MIT)/Plots/Limits/"+str(sys.argv[1])+"_prefit_postfit_"+region+".png")
  c.SaveAs(r"/local/ballen/Dropbox (MIT)/Plots/Limits/"+str(sys.argv[1])+"_prefit_postfit_"+region+".C")
...
plotPreFitPostFit(str(sys.argv[2]))
