...
def cmodel(cid,nam,_f,_fOut, out_ws, diag):
  ...
  for b in range(targetmc.GetNbinsX()):
    ewk_u = WZScales.Clone(); ewk_u.SetName("wz_weights_%s_ewk_W_%s_bin%d_Up"%(cid,cid,b))
    ewk_d = WZScales.Clone(); ewk_d.SetName("wz_weights_%s_ewk_W_%s_bin%d_Down"%(cid,cid,b))
    for j in range (targetmc.GetNbinsX()):
      if j==b:
...
def my_function(_wspace,_fin,_fOut,nam,diag):
  ...
  fkFactor = r.TFile.Open(os.path.join(os.environ['CMSSW_BASE'],"src/DmsMonoX","files/qcd_13TeV.root"))
